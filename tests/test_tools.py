...
def tmp_db():
    ...
    db.init_schema()
    # Seed some test data
    db.execute("INSERT INTO vgreen_telemetry (id, station_id, temperature_celsius, voltage, duty_cycle, status, fault_code) VALUES (1, 'VG-001', 45.0, 220.0, 75.0, 'OK', NULL)")
    db.execute("INSERT INTO vgreen_telemetry (id, station_id, temperature_celsius, voltage, duty_cycle, status, fault_code) VALUES (2, 'VG-001', 90.0, 220.0, 95.0, 'FAULT', 'THERMAL')")
    db.execute("INSERT INTO vgreen_telemetry (id, station_id, temperature_celsius, voltage, duty_cycle, status, fault_code) VALUES (3, 'VG-002', 35.0, 380.0, 50.0, 'OK', NULL)")
    db.execute("INSERT INTO vinfast_bms (id, vehicle_id, battery_soc, cell_temp_max, cell_temp_min, bms_fault_code) VALUES (1, 'VF8-001', 85.0, 35.0, 30.0, NULL)")
    db.execute("INSERT INTO vinfast_bms (id, vehicle_id, battery_soc, cell_temp_max, cell_temp_min, bms_fault_code) VALUES (2, 'VF8-002', 150.0, 55.0, 20.0, '0x4B')")
    db.execute("INSERT INTO xanhsm_feedback (id, review_text, rating, source) VALUES (1, 'Trạm sạc tốt lắm', 5.0, 'google_maps')")
    db.execute("INSERT INTO xanhsm_feedback (id, review_text, rating, source) VALUES (2, 'ko sac dc, tệ vl', 1.0, 'play_store')")
    db.execute("INSERT INTO xanhsm_trips (id, trip_id, distance_km, fare_vnd, duration_minutes) VALUES (1, 'T-001', 15.5, 85000, 25.0)")
    db.execute("INSERT INTO xanhsm_trips (id, trip_id, distance_km, fare_vnd, duration_minutes) VALUES (2, 'T-002', 600.0, 500000, 120.0)")
    yield db
    db.close()
    os.unlink(db_path)
...
def test_telemetry_has_sample(tmp_db, monkeypatch):
    ...
    tool = TelemetryQueryTool()
    result = tool.execute({})
    assert 'vgreen_sample' in result
...
def test_anomaly_z_score(tmp_db, monkeypatch):
    ...
    tool = AnomalyDetectorTool()
    result = tool.execute({'table_name': 'vgreen_telemetry', 'column_name': 'temperature_celsius'})
...
def test_anomaly_iqr(tmp_db, monkeypatch):
    monkeypatch.setattr('src.tools.anomaly_detector.get_db', lambda: tmp_db)
    tool = AnomalyDetectorTool()
    result = tool.execute({'table_name': 'vgreen_telemetry', 'column_name': 'temperature_celsius', 'method': 'iqr'})
    assert 'anomalies_found' in result
...
def test_anomaly_invalid_column(tmp_db, monkeypatch):
    monkeypatch.setattr('src.tools.anomaly_detector.get_db', lambda: tmp_db)
    tool = AnomalyDetectorTool()
    result = tool.execute({'table_name': 'vgreen_telemetry', 'column_name': 'nonexistent'})
    assert result['anomalies_found'] == 0

def test_anomaly_statistics(tmp_db, monkeypatch):
    monkeypatch.setattr('src.tools.anomaly_detector.get_db', lambda: tmp_db)
    tool = AnomalyDetectorTool()
    result = tool.execute({'table_name': 'vgreen_telemetry', 'column_name': 'voltage'})
...
def test_anomaly_bms_soc(tmp_db, monkeypatch):
    monkeypatch.setattr('src.tools.anomaly_detector.get_db', lambda: tmp_db)
    tool = AnomalyDetectorTool()
    result = tool.execute({'table_name': 'vinfast_bms', 'column_name': 'battery_soc'})
...
def test_propose_with_anomaly_findings():
    ...
    result = tool.execute({
        ...
        'anomaly_findings': {'anomalies_found': 5, 'statistics': {'mean': 50.0, 'std': 10.0}, 'column_name': 'temperature_celsius'}
    })
...
def test_profiler_basic(tmp_db, monkeypatch):
    ...
    result = tool.execute({'table_name': 'vgreen_telemetry'})
...
def test_profiler_column_stats(tmp_db, monkeypatch):
    ...
    tool = DataProfilerTool()
    result = tool.execute({'table_name': 'vgreen_telemetry'})
    temp_col = next((c for c in result['columns'] if c['name'] == 'temperature_celsius'), None)
...
def test_profiler_null_stats(tmp_db, monkeypatch):
    ...
    tool = DataProfilerTool()
    result = tool.execute({'table_name': 'vgreen_telemetry'})
    fault_col = next((c for c in result['columns'] if c['name'] == 'fault_code'), None)
...
def test_executor_dry_run(tmp_db, monkeypatch):
    ...
    # Insert a test rule
    tmp_db.execute("INSERT INTO quality_rules (id, rule_name, rule_type, rule_expression, confidence, status) VALUES ('r1', 'temp_check', 'range', 'temperature_celsius BETWEEN -10 AND 85', 0.95, 'approved')")
...
def test_executor_unapproved_rule(tmp_db, monkeypatch):
    monkeypatch.setattr('src.tools.rule_executor.get_db', lambda: tmp_db)
    tmp_db.execute("INSERT INTO quality_rules (id, rule_name, rule_type, rule_expression, confidence, status) VALUES ('r3', 'test', 'range', 'rating > 0', 0.9, 'proposed')")
    tool = RuleExecutorTool()
...
def test_executor_creates_audit_log(tmp_db, monkeypatch):
    monkeypatch.setattr('src.tools.rule_executor.get_db', lambda: tmp_db)
    tmp_db.execute("INSERT INTO quality_rules (id, rule_name, rule_type, rule_expression, confidence, status) VALUES ('r4', 'dist_check', 'range', 'distance_km < 500', 0.95, 'approved')")
    tool = RuleExecutorTool()
