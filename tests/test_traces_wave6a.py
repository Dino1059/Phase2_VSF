...
def test_normalize_trace_step_aliases_and_honest_summary():
    ...
    assert "Thought should not become Done" not in (card.get("summary_done") or "")
    assert card["summary_done"] != row[1]
...
def test_normalize_empty_output_does_not_invent_counts():
    ...
    assert "SHA-256" not in (card.get("summary_done") or "")
    assert "12 anomalies" not in (card.get("summary_done") or "")
