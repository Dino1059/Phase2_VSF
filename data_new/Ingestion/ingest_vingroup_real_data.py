...
# EDA 2026-08-14 phát hiện: fact_rides.csv KHÔNG phải USD thật (multi-country 230 quốc gia,
# fare_amount median 52.57 trên trip 10.3 miles ≈ 5.1 USD/mile — không khớp USD taxi).
# Nhân thẳng * USD_TO_VND cho ra ~81k VND/km, gấp ~5–10 lần taxi VN thật (12–22k).
# Scale factor để đưa median fare_per_km về ~18k VND (~ taxi Hà Nội thực tế).
# Chỉ áp dụng trong map_trips; KHÔNG scale ở map_charging (EVN tariff cố định).
FARE_SCALE_TO_VN_TAXI = 0.22
...
def map_charging_sessions(acn_json_paths: list[Path], fleet: pd.DataFrame) -> pd.DataFrame:
    ...
    for vin in fleet["vehicle_vin"].values:
        for day_idx in range(OBSERVATION_DAYS):
            ...
            for ev in ov_events:
                ...
                soft_overlap = False
                # EDA 2026-08-14: khong tron Tier R (kwh_consumed tu ACN) voi Tier S
                # (duration_mins tu state machine); ca 2 phai Tier R de power_kw co nghia.
                # duration_plug_min tinh tu disc - conn (ACN raw, Tier R).
                duration_plug_min_src = float((pd.to_datetime(src["disconnectTime"], utc=True, errors="coerce") -
                                                pd.to_datetime(src["connectionTime"], utc=True, errors="coerce")
                                                ).total_seconds() / 60.0)
                all_rows.append({
                    "vehicle_vin": vin,
                    ...
                    "duration_mins": duration_plug_min_src,
                    "kwh_consumed": float(src["kWhDelivered"]),
                    "power_kw": round(float(src["kWhDelivered"]) / max(0.05, duration_plug_min_src/60.0), 2),
            ...
            for ev in op_events:
                ...
                # 2026-08-08 v4: state machine guarantees disjoint -> soft_overlap_flag luon False
                soft_overlap = False
                # EDA 2026-08-14: Tier R consistent - duration_plug_min tu ACN raw.
                duration_plug_min_src = float((pd.to_datetime(src["disconnectTime"], utc=True, errors="coerce") -
                                                pd.to_datetime(src["connectionTime"], utc=True, errors="coerce")
                                                ).total_seconds() / 60.0)
                all_rows.append({
    ...
    manifest.tag("acn_charging_mapped", "duration_mins", "RD",
                 "Tier R: lay tu ACN raw (disconnectTime - connectionTime). EDA 2026-08-14: "
                 "truoc do dung tier S (state machine payload) gay Tier R+S mismatch khi tinh power_kw.")
...
def map_trips(fact_rides_path: Path, drivers_path: Path,
    ...
    # EDA 2026-08-14: fact_rides.ride_distance_miles co the la miles that hoac synthetic ratio.
    # Quy doi sang km de semantic-aligned voi taxi VN. Tier R gia tri goc, tier RD cho scale.
    df_meta["trip_distance_km"] = (fact["ride_distance_miles"].astype(float).values * 1.609344).round(3)
    ...
    for vin in fleet["vehicle_vin"].values:
        for day_idx in range(OBSERVATION_DAYS):
            ...
            for ev in trip_events:
                ...
                pickup_dt = ev.start
                dropoff_dt = ev.end
                # EDA 2026-08-14: fact_rides la synthetic khong phai USD that; scale theo
                # taxi VN thuc te (~12-22k VND/km). USD_TO_VND doc lap voi SCALE de
                # backward-compat cho cac consumer cu.
                fare_vnd = round(
                    float(src["fare_usd"]) * USD_TO_VND * FARE_SCALE_TO_VN_TAXI, 0
                )
                ...
                all_trip_rows.append({
                    ...
                    "assigned_day_index": day_idx,
                    "trip_distance_km": float(src["trip_distance_km"]),
                    "fare_amount": fare_vnd,
                    "currency_unverified": True,  # fare_amount la gia tri scaled tu synthetic source, khong phai VND tuyet doi tu he thong tax
                    "tip_amount": tip_vnd,
    ...
    manifest.tag("real_xanh_sm_trips", "assigned_day_index", "RD",
                 "Khoa ngay 0..14, derive tu schedule.")
    manifest.tag("real_xanh_sm_trips", "trip_distance_km", "RD",
                 "Quy doi tu fact_rides.ride_distance_miles (Tier R) x 1.609344. "
                 "EDA 2026-08-14: trc do ten la trip_miles nhung khong ro don vi that (multi-country); "
                 "rename + scale de semantic-aligned voi taxi VN.")
    manifest.tag("real_xanh_sm_trips", "fare_amount", "RD",
                 f"fact_rides.fare_amount x USD_TO_VND x "
                 f"FARE_SCALE_TO_VN_TAXI({FARE_SCALE_TO_VN_TAXI}). "
                 f"fact_rides la synthetic khong co don vi that (multi-country, "
                 f"EDA 2026-08-14), nen scale de map ve taxi VN thuc te ~15-25k VND/km.")
    manifest.tag("real_xanh_sm_trips", "currency_unverified", "S",
                 "Cot flag: True = fare_amount da scale tu synthetic (FARE_SCALE_TO_VN_TAXI), "
                 "khong phai VND tuyet doi tu he thong tax that.")
