...
Pipeline result cho VinGroup pilot được lưu theo `run_id` trong bảng runtime
`pipeline_runs`. Frontend đọc kết quả qua `GET /api/v1/pipeline/result/{run_id}`
để cập nhật RCA, telemetry, clean/quarantine split và SHA-256 manifest. Nếu
pipeline lỗi, run được đánh dấu `failed` và trả lỗi thay vì hiển thị dữ liệu giả.
